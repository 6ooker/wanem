...
@app.route("/bring_down/<interface>", methods=["POST"])
def bring_down(interface):
    interface = filter_interface_name(interface)
    command = f"ip link set dev {interface} down"
    print(command)
...
@app.route("/bring_up/<interface>", methods=["POST"])
def bring_up(interface):
    ...
    proc = subprocess.Popen(command)
    proc.wait()
    command = f"ip link set dev {interface} up"
    print(command)
    command = command.split(" ")
    proc = subprocess.Popen(command)
    proc.wait()
